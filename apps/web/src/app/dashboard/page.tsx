...
export default async function DashboardPage() {
  ...
  const [leads, listas, campanhas, disparados, sessoes] = await Promise.all([
  ...
  const nDisparados = disparados.count ?? 0;
  ...
  const stats: {
    ...
    { label: "Leads captados", value: nLeads, href: "/dashboard/lists", icon: Users, cor: "emerald" },
    { label: "Listas", value: listas.count ?? 0, href: "/dashboard/lists", icon: ListChecks, cor: "blue" },
    { label: "Campanhas", value: nCampanhas, href: "/dashboard/campaigns", icon: Send, cor: "violet" },
    { label: "Leads disparados", value: nDisparados, href: "/dashboard/crm", icon: Target, cor: "amber" },
  ];
  ...
  const passos: {
    ...
    { titulo: "Capte donos", desc: "Busque por segmento e traga o dono direto", href: "/dashboard/capture", icon: Radar, feito: nLeads > 0 },
    ...
    { titulo: "Acompanhe no CRM", desc: "Veja o status de cada lead", href: "/dashboard/crm", icon: KanbanSquare, feito: nDisparados > 0 },
  ];
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        >
          <Radar className="h-4 w-4" /> Captar donos
        </Link>
      </div>
      ...
      <div className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-sm text-neutral-500">{feitos} de {passos.length} concluídos</span>
        </div>
